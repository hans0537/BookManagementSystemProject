...
<script>
	...
	function fn_cancel_order(orderId, goodsId, orderGoodsQty){
		...
		if (answer) {
			...
			var i_orderGoodsQty = document.createElement("input");
			
			
		    i_orderId.name  = "orderId";
		    i_goodsId.name  = "goodsId";
		    i_orderGoodsQty.name = "orderGoodsQty";
		    
		    i_orderId.value		  = orderId;
		    i_goodsId.value  	  = goodsId;
		    i_orderGoodsQty.value = orderGoodsQty;
		       
		    formObj.appendChild(i_orderId);
		    formObj.appendChild(i_goodsId);
		    formObj.appendChild(i_orderGoodsQty);
	      ...
	      <c:forEach var="item" items="${myOrderList }"  varStatus="i">
	       <c:choose> 
			...
			<td>
			  <c:choose>
			   <c:when test="${item.deliveryState=='deliveryPrepared'}">
			       <input  type="button" onClick="fn_cancel_order('${item.orderId}', '${item.goodsId}', '${item.orderGoodsQty}')" value="주문취소"  />
			   </c:when>
			   <c:otherwise>
			      <input  type="button" onClick="fn_cancel_order('${item.orderId}', '${item.goodsId}', '${item.orderGoodsQty}')" value="주문취소" disabled />
			   </c:otherwise>
			  </c:choose>
			</td>
			</tr>
