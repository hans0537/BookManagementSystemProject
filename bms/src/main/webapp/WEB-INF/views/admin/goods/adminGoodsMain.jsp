...
<script>
	...
	function fn_delete_goods(goodsId ,goodsDelYn) {
	    location.href="${contextPath}/admin/goods/deleteGoods.do?goodsId=" + goodsId + "&goodsDelyn=" + goodsDelYn;
	}
     				...
     				<c:forEach var="item" items="${newGoodsList}">
						<tr>       
							...
							<td><strong>
								...
								<c:choose>
								  <c:when test="${item.goodsDelyn == 'Y' }">
								    <input type="button" value="상품복원" class="btn btn-primary btn-sm"  onClick="fn_delete_goods('${item.goodsId}','N')">   
								  </c:when>
								  <c:when test="${item.goodsDelyn == 'N' }">
								    <input type="button" value="상품삭제" class="btn btn-danger btn-sm"  onClick="fn_delete_goods('${item.goodsId}','Y')">
								  </c:when>
								</c:choose>
							</strong></td>
						</tr>
