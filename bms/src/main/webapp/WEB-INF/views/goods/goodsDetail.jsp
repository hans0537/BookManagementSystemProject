...
<script>
	...
	function fn_order_each_goods(goodsId,goodsTitle,goodsSalesPrice,fileName,goodsCnt){
		...
		if (orderGoodsQty.value * 1 > goodsCnt * 1 ){
			alert("재고가 부족합니다.");
			return;
		}
...
<body>
	...
	<div id="detail_table">
		<table>
			<tbody>
				...
				<tr>
					...
					<c:choose>
						<c:when test="${goods.goodsCnt == 0}">
							<td class="fixed">매진</td>
						</c:when>
						<c:otherwise>
							<td class="fixed">${goods.goodsCnt }</td>	
						</c:otherwise>
					</c:choose>
				</tr>
				<tr>
					...
					<c:choose>
						<c:when test="${goods.goodsCnt == 0}">
							<td class="fixed">
						      <select style="width: 60px;" id="orderGoodsQty" disabled>
							  	<option>1</option>
							  	<option>2</option>
							  	<option>3</option>
							  	<option>4</option>
				...
				<%-- 
				<tr>
					<td class="fixed">추천</td>
					<td class="fixed">
						<c:choose>
  							<c:when test="${memberId ne null}">
    							<a href='javascript: like_func();'><img src="${contextPath }/resources/image/blankHeart.png" width="15" height="15"  id='like_img'></a>
  							</c:when>
  							<c:otherwise>
    							<a href='javascript: login_need();'><img src="${contextPath }/resources/image/heart.jpg" width="15" height="15"></a>
  							</c:otherwise>
						</c:choose>

						<c:choose>
							<c:when test="${???.goodsLikeYn == 'Y' }">
								<input type="image" src="${contextPath }/resources/image/blankHeart.png" width="15" height="15" onClick="fn_like_goods()">   ${goods.goodsLike } 
							</c:when>
							<c:when test="${???.goodsLikeYn == 'N' }">
								<input type="image" src="${contextPath }/resources/image/heart.jpg" width="15" height="15" onClick="fn_like_goods()">   ${goods.goodsLike } 
							</c:when>
						</c:choose>
					</td>
				</tr>
		...
		<ul>
			<c:choose>
				<c:when test="${goods.goodsCnt == 0}">
					<li><a class="buy">재고대기중 </a></li>
				</c:when>
				<c:otherwise>
					<li><a class="buy" href="javascript:fn_order_each_goods('${goods.goodsId}','${goods.goodsTitle}','${goods.goodsSalesPrice}','${goods.goodsFileName}', '${goods.goodsCnt}');">구매하기 </a></li>
				</c:otherwise>
			</c:choose>
		</ul>
	</div>
	...
	<div id="container">
		...
		<div class="tab_container">
			...
			<div class="tab_content" id="tab6">
			    ...
			    <br>
			    <p>
					<label for="star">도서평점 &ensp;&ensp;&ensp;
				...
				<p>
					<textarea id="comment" name="comment" rows="3.5" cols="85" placeholder="이 책의 한줄평을 간단히 남겨주세요."></textarea>  
		  			<a class="btn" href="javascript:fn_add_comment();" >등록하기</a>
	 			</p>
			</div>
		</div>
	</div>
